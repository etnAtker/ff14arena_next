      ...
      nested: {
        ...
        StatusSnapshot: {
          fields: {
            ...
            multiplierPresent: { type: 'bool', id: 6 },
          },
        },
        CooldownState: {
          fields: {
        ...
        MapMarker: {
          fields: {
            ...
            radiusPresent: { type: 'bool', id: 7 },
            sizePresent: { type: 'bool', id: 8 },
          },
        },
        RingIndicatorRingSnapshot: {
          fields: {
        ...
        MechanicSnapshot: {
          fields: {
            ...
            filled: { type: 'bool', id: 34 },
            targetIdPresent: { type: 'bool', id: 35 },
            targetSlotPresent: { type: 'bool', id: 36 },
            damageTypePresent: { type: 'bool', id: 37 },
            colorPresent: { type: 'bool', id: 38 },
            stableIdPresent: { type: 'bool', id: 39 },
            shapePresent: { type: 'bool', id: 40 },
            markerShapePresent: { type: 'bool', id: 41 },
            targetRingColorPresent: { type: 'bool', id: 42 },
            radiusPresent: { type: 'bool', id: 43 },
            innerRadiusPresent: { type: 'bool', id: 44 },
            outerRadiusPresent: { type: 'bool', id: 45 },
            damagePresent: { type: 'bool', id: 46 },
            totalDamagePresent: { type: 'bool', id: 47 },
            botTransferCooldownMsPresent: { type: 'bool', id: 48 },
            transferCooldownMsPresent: { type: 'bool', id: 49 },
            directionPresent: { type: 'bool', id: 50 },
            anglePresent: { type: 'bool', id: 51 },
            lengthPresent: { type: 'bool', id: 52 },
            widthPresent: { type: 'bool', id: 53 },
            targetRingRadiusPresent: { type: 'bool', id: 54 },
            showLabelPresent: { type: 'bool', id: 55 },
            filledPresent: { type: 'bool', id: 56 },
            allowTransferPresent: { type: 'bool', id: 57 },
            allowDeadRetargetPresent: { type: 'bool', id: 58 },
...
const mapMarkerOptionalScalarKeys = ['radius', 'size'] as const;
const mechanicOptionalScalarKeys = [
  'targetId',
  'targetSlot',
  'damageType',
  'color',
...
function presenceKey(key: string): string {
  return `${key}Present`;
}

function hasOwnValue(record: ProtoRecord, key: string): boolean {
  return Object.prototype.hasOwnProperty.call(record, key) && record[key] !== undefined;
}

function hasProtoField(record: ProtoRecord, key: string): boolean {
  return record[presenceKey(key)] === true || hasOwnValue(record, key);
}

function withPresenceFields(record: ProtoRecord, keys: readonly string[]): ProtoRecord {
  for (const key of keys) {
    if (hasOwnValue(record, key)) {
      record[presenceKey(key)] = true;
    }
  }
...
function statusValue(value: unknown): StatusSnapshot {
  ...
  if (hasProtoField(record, 'multiplier')) {
    status.multiplier = numberValue(record.multiplier);
  }
...
function actorValue(value: unknown): BaseActorSnapshot {
  ...
  if (hasProtoField(record, 'online')) {
...
function mapMarkerValue(value: unknown): MapMarker {
  ...
  if (hasProtoField(record, 'radius')) {
    marker.radius = numberValue(record.radius);
  }

  if (hasProtoField(record, 'size')) {
    marker.size = numberValue(record.size);
  }
...
function mechanicValue(value: unknown): MechanicSnapshot {
  ...
  copyOptionalBoolean(record, mechanic, 'filled');
...
function eventToProto(event: SimulationEvent): ProtoRecord {
  ...
  switch (event.type) {
    ...
    case 'aoeSpawned':
      return {
        ...proto,
        mechanic: mechanicToProto(event.payload),
      };
    case 'aoeResolved':
      return {
        ...proto,
    ...
    case 'statusApplied':
      return {
        ...
        targetId: event.payload.targetId,
        targetName: event.payload.targetName,
        status: statusToProto(event.payload.status),
      };
    case 'actorDied':
      return {
        ...proto,
...
function copyOptionalBoolean(source: ProtoRecord, target: ProtoRecord, key: string): void {
  if (hasProtoField(source, key)) {
    target[key] = booleanValue(source[key]);
  }
}

function statusToProto(status: StatusSnapshot): ProtoRecord {
  return withPresenceFields({ ...status }, statusOptionalScalarKeys);
}

function actorToProto(actor: BaseActorSnapshot): ProtoRecord {
  return withPresenceFields(
    {
      ...actor,
      slot: actor.slot ?? undefined,
      statuses: actor.statuses.map(statusToProto),
      deathReason: actor.deathReason ?? undefined,
      lastDamageSource: actor.lastDamageSource ?? undefined,
    },
    actorOptionalScalarKeys,
  );
}

function bossToProto(boss: BossSnapshot): ProtoRecord {
  return {
    actor: actorToProto(boss),
    castBar: boss.castBar ?? undefined,
    targetRingRadius: boss.targetRingRadius,
  };
}

function mapMarkerToProto(marker: MapMarker): ProtoRecord {
  return withPresenceFields({ ...marker }, mapMarkerOptionalScalarKeys);
}

function mechanicToProto(mechanic: MechanicSnapshot): ProtoRecord {
...
function snapshotToProto(snapshot: SimulationSnapshot): ProtoRecord {
  return {
    ...snapshot,
    mapMarkers: snapshot.mapMarkers.map(mapMarkerToProto),
