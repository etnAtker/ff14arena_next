...
test('凯夫卡一运：调试 Bot controller 已登记', () => {
...
test('凯夫卡一运：最后一轮消灭之脚锁定后8个Bot统一躲避', () => {
  const controller = getBattleBotController('kefka_p2_first_forsaken');
  const footSources = [
    { center: { x: 0, y: 0 }, direction: 0 },
    { center: { x: 0, y: 0 }, direction: 0 },
    { center: { x: 0, y: 0 }, direction: 0 },
    { center: { x: 0, y: 0 }, direction: 0 },
  ];
  const expectedTarget = KEFKA_P2_FIRST_FORSAKEN_TESTING.getFinalFootDodgeTarget(footSources);
...
test('凯夫卡一运：最后一轮消灭之脚躲避点不在锁定半场刀内', () => {
  const footSources = [
    { center: { x: 0, y: 0 }, direction: 0 },
    { center: { x: 2, y: 0 }, direction: 0 },
    { center: { x: -2, y: 0 }, direction: 0 },
    { center: { x: 0, y: 2 }, direction: 0 },
  ];
  const target = KEFKA_P2_FIRST_FORSAKEN_TESTING.getFinalFootDodgeTarget(footSources);

  for (const source of footSources) {
    const targetDirection = Math.atan2(target.y - source.center.y, target.x - source.center.x);
    let angleDiff = Math.abs(targetDirection - source.direction) % (Math.PI * 2);

    if (angleDiff > Math.PI) {
      angleDiff = Math.PI * 2 - angleDiff;
    }

    assert.ok(angleDiff > Math.PI / 2);
  }
});
