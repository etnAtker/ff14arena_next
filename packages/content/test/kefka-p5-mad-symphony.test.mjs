...
const DH_SLOTS = [...HEALER_SLOTS, ...DPS_SLOTS];
...
test('凯夫卡P5癫狂交响曲：首轮蓝圈随机3名非T，次轮蓝圈选择最近3名玩家', () => {
  withMockedRandom(createSeededRandomValues(51, 100), () => {
    ...
    const assignments = getAssignments(firstSnapshot);
    const firstDhTargetIds = assignments.firstDhTargetIds;
    const dhActorIds = DH_SLOTS.map((slot) => getActorBySlot(firstSnapshot, slot).id);
    const healerActorIds = HEALER_SLOTS.map((slot) => getActorBySlot(firstSnapshot, slot).id);

    assert.equal(new Set(firstDhTargetIds).size, 3);
    assert.ok(firstDhTargetIds.every((actorId) => dhActorIds.includes(actorId)));
    assert.ok(firstDhTargetIds.some((actorId) => healerActorIds.includes(actorId)));
    ...
    assert.equal(assignments.nuclearTargetId, 'player_MT');
    assert.equal(assignments.holyTargetId, 'player_ST');
...
test('凯夫卡P5癫狂交响曲：首轮命中后赋予核爆和神圣Buff', () => {
  withMockedRandom(createSeededRandomValues(52, 100), () => {
    ...
    assert.equal(nuclearTarget.slot, 'MT');
    assert.equal(holyTarget.slot, 'ST');
