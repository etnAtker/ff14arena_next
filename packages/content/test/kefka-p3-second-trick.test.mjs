...
function getKefkaMarkers(snapshot) {
...
test('初始目标分配时MT和ST不会获得相同时间的混沌之土', () => {
  withMockedRandom(createSeededRandomValues(13, 64), () => {
    const simulation = createKefkaP3SecondSimulation();
    ...
    const mtStatusId = targetStatusIds.find((statusId) => hasStatus(mt, statusId));
    const stStatusId = targetStatusIds.find((statusId) => hasStatus(st, statusId));

    assert.ok(mtStatusId);
    assert.ok(stStatusId);
    assert.notEqual(mtStatusId, stStatusId);
  });
});

test('头标按H1 H2 D1 D2 D3 D4 ST MT顺位分配编号', () => {
  withMockedRandom(
    ...
    () => {
      ...
      const h2 = getActorBySlot(snapshot, 'H2');
      const d1 = getActorBySlot(snapshot, 'D1');
      ...
      assert.equal(hasStatus(d1, SECOND_TARGET_STATUS_ID), true);
      ...
      const stMarker = getActorMarker(snapshot, st, SECOND_TARGET_MARKER_COLOR);
      const d1Marker = getActorMarker(snapshot, d1, SECOND_TARGET_MARKER_COLOR);
      const d2Marker = getActorMarker(snapshot, d2, SECOND_TARGET_MARKER_COLOR);

      assert.equal(h1Marker.label, '1');
      assert.equal(h2Marker.label, '2');
      assert.equal(mtMarker.label, '3');
      assert.equal(d1Marker.label, '1');
      assert.equal(d2Marker.label, '2');
      assert.equal(stMarker.label, '3');
    },
  );
});

test('奶妈同组时按H1到H2优先获得小编号', () => {
  withMockedRandom(
    createInitialAssignmentRandomValues([0, 0, 0, 0.999, 0.999, 0.999, 0.999, 0.999]),
    () => {
      const simulation = createKefkaP3SecondSimulation();
      ...
      assert.equal(hasStatus(h1, FIRST_TARGET_STATUS_ID), true);
      assert.equal(hasStatus(h2, FIRST_TARGET_STATUS_ID), true);
      assert.equal(hasStatus(d1, FIRST_TARGET_STATUS_ID), true);

      assert.equal(getActorMarker(snapshot, h1, FIRST_TARGET_MARKER_COLOR).label, '1');
      assert.equal(getActorMarker(snapshot, h2, FIRST_TARGET_MARKER_COLOR).label, '2');
      assert.equal(getActorMarker(snapshot, d1, FIRST_TARGET_MARKER_COLOR).label, '3');
    },
  );
});

test('红色二人组也按固定顺位分配编号', () => {
  withMockedRandom(
    createInitialAssignmentRandomValues([0.999, 0.999, 0, 0.5, 0.5, 0.5, 0.5, 0.5]),
    () => {
      const simulation = createKefkaP3SecondSimulation();
      ...
      const d1 = getActorBySlot(snapshot, 'D1');
      const d2 = getActorBySlot(snapshot, 'D2');
      assert.equal(hasStatus(d1, THIRD_TARGET_STATUS_ID), true);
      assert.equal(hasStatus(d2, THIRD_TARGET_STATUS_ID), true);

      assert.equal(getActorMarker(snapshot, d1, THIRD_TARGET_MARKER_COLOR).label, '1');
      assert.equal(getActorMarker(snapshot, d2, THIRD_TARGET_MARKER_COLOR).label, '2');
    },
  );
});
...
test('第一次黑洞生成固定源连线并提前显示射线预兆', () => {
  withMockedRandom(createSeededRandomValues(22, 128), () => {
    const simulation = createKefkaP3SecondSimulation();
    const blackHoleSpawnAt = ZERO_AT + 16_400 + 2_700;
...
test('诅咒敕令读条开始时随机锁定玩家方向', () => {
  withMockedRandom(
    ...
    () => {
      ...
      advanceTo(simulation, ZERO_AT + 39_750);
      ...
      const locked = snapshot.scriptState[`kefkaP3Second:curse:${ZERO_AT + 39_800}`];
      assert.ok(locked);
...
test('暴雷锁定最近目标时艾克斯迪斯会转向目标', () => {
  withMockedRandom(createSeededRandomValues(34, 128), () => {
    const simulation = createKefkaP3SecondSimulation();
    const firstThunderResolveAt = ZERO_AT + 31_800 + THUNDER_CAST_MS;
...
test('第三组黑洞在83.9秒生成并允许同一玩家持有多根连线', () => {
  withMockedRandom(createSeededRandomValues(66, 512), () => {
    const simulation = createKefkaP3SecondSimulation();

    advanceTo(simulation, ZERO_AT + 83_900);
...
test('第二组三连黑洞出现后7.1秒首射，后续每5.1秒射击', () => {
  withMockedRandom(createSeededRandomValues(67, 512), () => {
    const simulation = createKefkaP3SecondSimulation();
    const blackHoleSpawnAt = ZERO_AT + 49_600;
    const shotAts = [
      blackHoleSpawnAt + PERSISTENT_BLACK_HOLE_FIRST_SHOT_DELAY_MS,
      blackHoleSpawnAt +
        PERSISTENT_BLACK_HOLE_FIRST_SHOT_DELAY_MS +
        PERSISTENT_BLACK_HOLE_REPEAT_INTERVAL_MS,
      blackHoleSpawnAt +
    ...
    let snapshot = simulation.getSnapshot();
    assert.equal(getMechanics(snapshot, 'fieldMarker', '黑洞').length, 3);
    assert.equal(getMechanics(snapshot, 'tether', '黑洞连线').length, 3);

    for (const shotAt of shotAts) {
      advanceTo(simulation, shotAt - TELEGRAPH_MS);
      snapshot = simulation.getSnapshot();
...
test('103.1秒凯夫卡先出现，109.1秒才开始响亮亮耳光读条', () => {
  withMockedRandom(createSeededRandomValues(77, 512), () => {
    const simulation = createKefkaP3SecondSimulation();

    advanceTo(simulation, ZERO_AT + 104_100);
    ...
    advanceTo(simulation, ZERO_AT + 109_100);
...
test('凯夫卡释放结束后按日志观察点和2秒空窗切换', () => {
  withMockedRandom(createSeededRandomValues(79, 512), () => {
    ...
    advanceTo(simulation, ZERO_AT + 37_800);
    assert.equal(hasKefkaMarker(simulation.getSnapshot()), false);

    advanceTo(simulation, ZERO_AT + 39_800);
    assert.ok(hasKefkaMarker(simulation.getSnapshot()));

    advanceTo(simulation, ZERO_AT + 65_000);
    assert.equal(hasKefkaMarker(simulation.getSnapshot()), false);

    advanceTo(simulation, ZERO_AT + 67_000);
    assert.ok(hasKefkaMarker(simulation.getSnapshot()));

    advanceTo(simulation, ZERO_AT + 101_100);
    assert.equal(hasKefkaMarker(simulation.getSnapshot()), false);

    advanceTo(simulation, ZERO_AT + 103_100);
    assert.ok(hasKefkaMarker(simulation.getSnapshot()));

    advanceTo(simulation, LATE_TRUE_SELF_SPAWN_AT - 2_000);
    assert.equal(hasKefkaMarker(simulation.getSnapshot()), false);

    advanceTo(simulation, LATE_TRUE_SELF_SPAWN_AT);
    assert.equal(getKefkaMarkers(simulation.getSnapshot()).length, 1);

    advanceTo(simulation, LATE_TRUE_SELF_DESPAWN_AT);
    assert.equal(hasKefkaMarker(simulation.getSnapshot()), false);
  });
});
...
test('本色出演的我使用50m矩形范围', () => {
  withMockedRandom(createSeededRandomValues(78, 512), () => {
    ...
    advanceTo(simulation, ZERO_AT + 67_000 + 1_700 + TRUE_SELF_CAST_MS - TELEGRAPH_MS);
...
test('107.6秒卡奥斯随机释放经度聚爆或纬度聚爆', () => {
  withMockedRandom(createSeededRandomValues(88, 512), () => {
    const simulation = createKefkaP3SecondSimulation();

    advanceTo(simulation, ZERO_AT + 107_550);
    ...
    let snapshot = simulation.getSnapshot();
    const stateKey = `kefkaP3Second:chaosExplosion:${ZERO_AT + 107_600}`;
    ...
    advanceTo(simulation, ZERO_AT + 107_600 + CHAOS_EXPLOSION_CAST_MS - TELEGRAPH_MS);
...
test('124.5秒两黑洞判定同时凯夫卡出现并读条本色出演的我', () => {
...
test('131.6秒会同 tick 判定本色出演的我和最后一次黑洞射线', () => {
...
test('145.1秒和148.1秒脚下圈会生成8个5米预警并按记录位置判定', () => {
...
test('终盘两次分摊目标会在支援组和DPS之间交替，成功分摊会获得2.5秒易伤', () => {
  withMockedRandom(createSeededRandomValues(304, 512), () => {
    ...
    const secondMarker = getMechanics(snapshot, 'actorMarker').find(
      (mechanic) => mechanic.label === '终盘分摊',
    );

    assert.deepEqual([firstState.roleGroup, secondState.roleGroup].sort(), ['dps', 'support']);
    assert.notEqual(firstState.roleGroup, secondState.roleGroup);
    assert.equal(secondMarker?.markerShape, 'stackCircle');
    assert.equal(secondMarker?.radius, TERMINAL_SHARE_RADIUS);
    assert.equal(secondMarker?.color, TERMINAL_SHARE_MARKER_COLOR);
...
test('151.1秒同 tick 会完成第一次分摊、第二次点名和首座终盘塔', () => {
...
test('161.1秒会在两次分摊记录位置造成返还即死范围', () => {
  withMockedRandom(createSeededRandomValues(309, 512), () => {
    ...
    let snapshot = simulation.getSnapshot();
    const firstShareState = getTerminalShareState(snapshot, 0);
    const stalagmiteTelegraphs = getMechanics(snapshot, 'circleTelegraph', '终盘石笋范围');

    assert.ok(firstShareState.center);
    assert.equal(stalagmiteTelegraphs.length, 2);
    assert.ok(
      stalagmiteTelegraphs.every(
        (mechanic) =>
          mechanic.radius === TERMINAL_SHARE_RADIUS &&
          mechanic.color === TERMINAL_SHARE_MARKER_COLOR,
      ),
    );
...
test('P3二运战斗会在165秒完成', () => {
  ...
  assert.equal(COMPLETE_AT, 165_000);
...
test('凯夫卡P3二运：开战跳到第三组黑洞期间会重建三根连线', () => {
  withMockedRandom(createSeededRandomValues(205, 512), () => {
    const blackHoleSpawnAt = ZERO_AT + 83_900;
    const simulation = createKefkaP3SecondSimulation({ startTimeMs: blackHoleSpawnAt + 100 });
    const snapshot = simulation.getSnapshot();
    const blackHoles = snapshot.mechanics.filter(
      (mechanic) => mechanic.kind === 'fieldMarker' && mechanic.label === '黑洞',
...
test('凯夫卡P3二运：开战跳到后半聚爆读条中会恢复读条和状态', () => {
  withMockedRandom(createSeededRandomValues(204, 512), () => {
    const castStartAt = ZERO_AT + 107_600;
