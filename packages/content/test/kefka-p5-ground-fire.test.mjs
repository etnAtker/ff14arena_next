...
const OBSERVED_FIRE_POSITION_SAMPLE = [
  [-15, -20],
  [-30, -5],
  [0, 0],
  [-25, 0],
...
test('凯夫卡P5地火：左右起始线按5m网格生成，6个起点斜向间隔5√2m', () => {
  for (const side of ['left', 'right']) {
    ...
    assertNear(START_LINE_POINT_GAP, FIRE_GRID_STEP * Math.SQRT2, `${side} 起点斜向间隔`);
    ...
    for (let index = 1; index < starts.length; index += 1) {
      ...
      assert.equal(Math.abs(starts[index].x - starts[index - 1].x), FIRE_GRID_STEP);
      assert.equal(starts[index].y - starts[index - 1].y, FIRE_GRID_STEP);
    }
  }
});

test('凯夫卡P5地火：每条地火向场中方向步进7次，每次沿5m网格斜向前进', () => {
  for (const side of ['left', 'right']) {
    for (const number of [1, 2, 3, 4, 5, 6]) {
      ...
      for (let hitIndex = 1; hitIndex < HIT_COUNT; hitIndex += 1) {
        ...
        assert.equal(
          Math.abs(
            getFireHitPosition(side, number, hitIndex).x -
              getFireHitPosition(side, number, hitIndex - 1).x,
          ),
          FIRE_GRID_STEP,
          `${side}${number} 第${hitIndex}次 x 网格步进`,
        );
        assert.equal(
          getFireHitPosition(side, number, hitIndex).y -
            getFireHitPosition(side, number, hitIndex - 1).y,
          FIRE_GRID_STEP,
          `${side}${number} 第${hitIndex}次 y 网格步进`,
        );
      }
    }
  }
...
test('凯夫卡P5地火：5m网格轨迹与观测样本除噪音点外一致', () => {
  const generatedPositions = [];

  for (const side of ['left', 'right']) {
    for (const number of [1, 2, 3, 4, 5, 6]) {
      for (let hitIndex = 0; hitIndex < HIT_COUNT; hitIndex += 1) {
        generatedPositions.push(getFireHitPosition(side, number, hitIndex));
      }
    }
  }

  const generatedPositionKeys = new Set(generatedPositions.map(positionKey));
  const samplePositionKeys = new Set(OBSERVED_FIRE_POSITION_SAMPLE.map(([x, y]) => `${x},${y}`));
  const sampleOnlyKeys = [...samplePositionKeys].filter((key) => !generatedPositionKeys.has(key));
  const generatedOnlyKeys = [...generatedPositionKeys].filter(
    (key) => !samplePositionKeys.has(key),
  );

  assert.equal(generatedPositionKeys.size, 48);
  assert.equal(samplePositionKeys.size, 48);
  assert.deepEqual(sampleOnlyKeys.sort(), ['0,-10', '0,0']);
  assert.deepEqual(generatedOnlyKeys.sort(), ['-15,0', '5,0']);
});
  ...
  withMockedRandom([0, 0, 0.99, 0.99], () => {
    ...
    assert.ok(
      initialTelegraphs.every(
        (mechanic) =>
          ...
          mechanic.direction === LEFT_FIRE_DIRECTION &&
    ...
    const rightFirstTelegraphs = rightFirstSnapshot.mechanics.filter(
      (mechanic) =>
        ...
        mechanic.direction === RIGHT_FIRE_DIRECTION &&
