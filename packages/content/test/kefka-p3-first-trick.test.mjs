...
test('凯夫卡P3一运：暴雷在MT到场中的射线上锁定位置并生成艾克斯德司', () => {
  withMockedRandom(createSeededRandomValues(23, 100), () => {
    ...
    const exdeathMarker = castSnapshot.stageActors.find(
      (mechanic) => mechanic.label === '艾克斯德司',
    );
...
test('凯夫卡P3一运：第二组暴雷选择艾克斯德司最近目标并判定5m范围', () => {
  withMockedRandom(createSeededRandomValues(23, 100), () => {
    ...
    const lockedExdeathMarker = firstResolvedSnapshot.stageActors.find(
      (mechanic) => mechanic.label === '艾克斯德司',
    );
    assertExdeathMarker(
      lockedExdeathMarker,
...
test('凯夫卡P3一运：37秒到61秒之间艾克斯德司每秒按MT位置重定位', () => {
  withMockedRandom(createSeededRandomValues(35, 200), () => {
    ...
    snapshot = simulation.getSnapshot();
    let exdeathMarkers = snapshot.stageActors.filter((mechanic) => mechanic.label === '艾克斯德司');
    ...
    snapshot = simulation.getSnapshot();
    exdeathMarkers = snapshot.stageActors.filter((mechanic) => mechanic.label === '艾克斯德司');
    assert.equal(exdeathMarkers.length, 1);
...
test('凯夫卡P3一运：卡奥斯3秒出现并每秒按ST位置重定位', () => {
  ...
  let snapshot = simulation.getSnapshot();
  let chaosMarkers = snapshot.stageActors.filter((mechanic) => mechanic.label === '卡奥斯');
  ...
  snapshot = simulation.getSnapshot();
  chaosMarkers = snapshot.stageActors.filter((mechanic) => mechanic.label === '卡奥斯');
  assert.equal(chaosMarkers.length, 1);
...
test('凯夫卡P3一运：艾克斯德司3秒出现并在暴雷前按MT位置重定位', () => {
  const simulation = createKefkaP3Simulation();
  ...
  assert.equal(exdeathMarkers.length, 1);
  const initialExdeathMarkerId = exdeathMarkers[0].id;
  assertExdeathMarker(exdeathMarkers[0], { x: 0, y: 0 }, CHAOS_REPOSITION_START_AT);
  ...
  assert.equal(exdeathMarkers.length, 1);
  assert.equal(exdeathMarkers[0].id, initialExdeathMarkerId);
  assertClosePoint(getBurstCenter(snapshot), expectedCenter);
  assertExdeathMarker(
    exdeathMarkers[0],
    expectedCenter,
    CHAOS_REPOSITION_START_AT + EXDEATH_REPOSITION_INTERVAL_MS,
    angleTo(expectedCenter, mtPosition),
  );
});

test('凯夫卡P3一运：关闭MT拉艾克斯迪斯后小Boss追踪目标互换', () => {
  const simulation = createKefkaP3Simulation({
    roomOptions: {
  ...
  const mt = getActorBySlot(snapshot, 'MT');
  const mtPosition = { x: 12, y: 0 };
  const expectedChaosCenter = calculateChaosCenter({ x: 0, y: 0 }, mtPosition);
  submitPose(simulation, mt, mtPosition);

  advanceTo(simulation, CHAOS_REPOSITION_START_AT);

  snapshot = simulation.getSnapshot();
  assertClosePoint(getChaosCenter(snapshot), expectedChaosCenter);

  const st = getActorBySlot(snapshot, 'ST');
  const stPosition = { x: -12, y: 0 };
  const expectedBurstCenter = calculateBurstCenter(stPosition);
  submitPose(simulation, st, stPosition);

  advanceTo(simulation, BURST_CAST_START_AT);

  snapshot = simulation.getSnapshot();
  assertClosePoint(getBurstCenter(snapshot), expectedBurstCenter);
});
...
function runChaosExplosionTest({
  ...
  withMockedRandom([randomValue], () => {
    ...
    const castSnapshot = simulation.getSnapshot();
    const chaosState = castSnapshot.scriptState['kefkaP3:chaosExplosion'];
    const chaosMarker = castSnapshot.stageActors.find((mechanic) => mechanic.label === '卡奥斯');
    assert.ok(chaosState);
...
test('凯夫卡P3一运：超级跳锁定最远玩家位置并在67s结算11m范围', () => {
  withMockedRandom(createSeededRandomValues(30, 100), () => {
    ...
    const originalChaosMarker = setupSnapshot.stageActors.find(
    ...
    const jumpedChaosMarker = resolvedSnapshot.stageActors.find(
      (mechanic) => mechanic.label === '卡奥斯' && mechanic.resolveAt === COMPLETE_AT,
    );
...
test('凯夫卡P3一运：真空波在61s锁定并重建艾克斯德司位置', () => {
  withMockedRandom(createSeededRandomValues(27, 100), () => {
    ...
    const oldExdeathMarker = beforeCastSnapshot.stageActors.find(
      (mechanic) => mechanic.label === '艾克斯德司',
    );
    ...
    const castSnapshot = simulation.getSnapshot();
    const newExdeathMarker = castSnapshot.stageActors.find(
      (mechanic) => mechanic.label === '艾克斯德司' && mechanic.resolveAt === COMPLETE_AT,
    );
