...
test('凯夫卡P3一运：水buff两次消除后延迟1500ms点名最近2人', () => {
  withMockedRandom(createSeededRandomValues(24, 100), () => {
    ...
    const candidatePositions = candidateActors.map((_, index) =>
    ...
    assert.equal(waterTelegraphs.length, 2);
    assert.ok(waterTelegraphs.every((mechanic) => mechanic.color === WATER_TELEGRAPH_COLOR));
    assert.ok(
      candidatePositions.every((position) =>
        waterTelegraphs.some((mechanic) => distance(mechanic.center, position) < 0.001),
      ),
    );
...
test('凯夫卡P3一运：风消除后延迟1500ms判定2人分摊，少于2人即死', () => {
  withMockedRandom(createSeededRandomValues(24, 100), () => {
    const simulation = createKefkaP3Simulation();
...
test('凯夫卡P3一运：完成时间整体提前到55500ms', () => {
  withMockedRandom(createSeededRandomValues(26, 100), () => {
    const simulation = createKefkaP3Simulation();

    assert.equal(COMPLETE_AT, 55_500);
