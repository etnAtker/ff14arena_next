...
const TOWER_RING_KEYS = ['-12,-3', '-3,-12', '3,-12', '12,-3', '12,3', '3,12', '-3,12', '-12,3'];
...
const SHOCKWAVE_CARDINAL_KEYS = ['0,-15', '15,0', '0,15', '-15,0'];
...
test('欧米茄绝境战 P1 循环程序：全机器人按随机塔位脚本完成机制', () => {
  ...
  withMockedRandom(randomValues, () => {
    ...
    assert.equal(initialSnapshot.bossTargetRingRadius, 12);
    assert.deepEqual(
    ...
    assert.equal(initialSnapshot.mapMarkers[0]?.radius, 1.25);
    assert.equal(initialSnapshot.mapMarkers[1]?.size, 2.2);
    assert.equal(getDistance(initialSnapshot.mapMarkers[0].position, { x: 0, y: 0 }), 13);
...
test('欧米茄绝境战 P1 循环程序：Bot 接到当前轮连线后先在 5m 点等待', () => {
  withMockedRandom(createSeededRandomValues(8, 32), () => {
    ...
    assert.equal(checkedStagingPoint, true, 'Bot 接到线后应先停在距离中心约 5m 的中转点');
...
test('欧米茄绝境战 P1 循环程序：更晚编号提前踩塔会消耗自身状态', () => {
  withMockedRandom([0.91, 0.17, 0.73, 0.44, 0.28, 0.62, 0.05], () => {
    ...
    submitPose(simulation, lateActor, tower.center);
    simulation.tick(50);
...
test('欧米茄绝境战 P1 循环程序：同一塔多名有效玩家会全部消耗编号状态', () => {
  withMockedRandom([0.91, 0.17, 0.73, 0.44, 0.28, 0.62, 0.05], () => {
    const simulation = createTopProgramLoopSimulation();
    ...
    const snapshotAfterAssignment = simulation.getSnapshot();
    const firstRound = snapshotAfterAssignment.scriptState['top:rounds']?.[0];
    assert.ok(firstRound);

    const handlers = snapshotAfterAssignment.actors.filter((actor) =>
      actor.statuses.some((status) => status.id.startsWith('program_loop_')),
    );
    assert.equal(handlers.length >= 3, true);

    const consumedStatusIds = handlers.slice(0, 2).map((actor) => {
      const status = actor.statuses.find((candidate) => candidate.id.startsWith('program_loop_'));
      assert.ok(status);
      return status.id;
    });

    submitPose(simulation, handlers[0], firstRound.towerPositions[0]);
    submitPose(simulation, handlers[1], firstRound.towerPositions[0]);
    submitPose(simulation, handlers[2], firstRound.towerPositions[1]);

    advanceTo(simulation, 17_600);

    const snapshot = simulation.getSnapshot();
    assert.equal(snapshot.failureReasons.includes('第 1 轮塔爆炸'), false);

    handlers.slice(0, 2).forEach((handler, index) => {
      const actorAfterTower = snapshot.actors.find((actor) => actor.id === handler.id);
      assert.ok(actorAfterTower);
      assert.equal(
        actorAfterTower.statuses.some((status) => status.id === consumedStatusIds[index]),
        false,
      );
      assert.equal(
        actorAfterTower.statuses.some((status) => status.id === 'twice_come_ruin'),
        true,
      );
    });
  });
...
test('欧米茄绝境战 P1 循环程序：没有编号状态玩家踩塔会触发塔爆炸团灭', () => {
  withMockedRandom([0.91, 0.17, 0.73, 0.44, 0.28, 0.62, 0.05], () => {
    const simulation = createTopProgramLoopSimulation();
    ...
    const snapshot = simulation.getSnapshot();
    assert.equal(snapshot.latestResult?.outcome, 'failure');
    assert.ok(snapshot.failureReasons.includes('第 1 轮塔爆炸'));
    assert.equal(
      snapshot.actors.every((actor) => !actor.alive),
      true,
    );
    assert.equal(
      snapshot.actors.every((actor) => actor.deathReason === '塔爆炸'),
      true,
    );
  });
});
...
test('欧米茄绝境战 P1 循环程序：循环程序状态到期未消除会触发遗忘死亡', () => {
  withMockedRandom([0.91, 0.17, 0.73, 0.44, 0.28, 0.62, 0.05], () => {
    ...
    const snapshotAfterAssignment = simulation.getSnapshot();
    const firstActor = snapshotAfterAssignment.actors.find((actor) =>
      actor.statuses.some((status) => status.id === 'program_loop_1'),
    );
    ...
    const substituteHandlers = snapshotAfterAssignment.actors.filter(
      (actor) =>
        actor.id !== firstActor.id &&
        actor.statuses.some((status) => status.id.startsWith('program_loop_')),
    );
    assert.equal(substituteHandlers.length >= 2, true);

    submitPose(simulation, firstActor, { x: 0, y: 0 });
    submitPose(simulation, substituteHandlers[0], firstRound.towerPositions[0]);
    submitPose(simulation, substituteHandlers[1], firstRound.towerPositions[1]);
