...
2. 其他玩家通过 Socket 加入房间或直接加入观战
3. 房主选择 battle，空槽自动补 Bot，并建立等待态权威模拟
4. 房主开始倒计时，倒计时结束后服务端把同一个权威模拟切入 `running`
5. 运行中房主可以快速失败，服务端立即把本轮结果结算为失败并回到等待态
6. 模拟结束后，服务端重建等待态权威模拟并保留上一轮结果
...
正式开战时，服务端保留倒计时结束时的玩家位置和朝向，并把战斗时间从 `0ms` 开始推进。
