...
- `23000ms` 暴雷以锁定位置为中心判定半径 `11m` 白色范围，命中玩家获得永久 `死亡` buff，并记录失败来源 `被暴雷命中`
- `27000ms` 开始第二组读条“暴雷”，`33000ms` 判定一次，`36000ms` 再判定一次；每次都按距离 `艾克斯德司` 最近的存活玩家重新选目标
- 第二组暴雷以目标当前位置为中心判定半径 `5m` 白色范围，范围内 `MT/ST` 受到普通机制伤害，非 `MT/ST` 玩家获得永久 `死亡` buff，并记录失败来源 `被暴雷命中`
...
- 聚爆在 `46000ms` 与 `48000ms` 各以锁定的 `卡奥斯` 位置为中心判定一次两片相对 `90°`、`60m` 扇形；经度聚爆顺序为前后再左右，纬度聚爆顺序为左右再前后，命中玩家获得永久 `死亡` buff，并记录失败来源 `被扇形命中`
- `61000ms` 超级跳锁定距离场中心最远的存活玩家当前位置；距离相同时按 `PARTY_SLOT_ORDER` 稳定选择，锁定后玩家移动不改变结算点
- `67000ms` 超级跳将 `卡奥斯` 瞬移到锁定位置并重建同名敌人标记和红色 `6m` 目标圈，短暂显示半径 `11m` 圆形范围
- 超级跳范围内玩家获得永久 `死亡` buff，并记录失败来源 `被超级跳命中`；该机制不击退、不消耗风类状态、不排队风分摊
...
- 顺位本人距离矩形起点小于 `40m` 时获得永久 `死亡` buff，失败来源 `麻将距离过近`；其它玩家被矩形命中时获得永久 `死亡` buff，失败来源 `麻将被其它人的矩形命中`
...
- 风分摊追击在累计后延迟 `1500ms` 统一判定；每次选中距离水元素块最近的不同玩家，结算半径 `6m` 的 `2` 人分摊，结算时短暂显示绿色范围
- 风分摊少于 `2` 人时记录失败，并使目标获得永久 `死亡` buff
- 机制伤害命中已有易伤玩家时获得永久 `死亡` buff，否则造成伤害并赋予 `3s` 易伤；该 `死亡` buff 只作为测试标记，玩家仍保持存活并继续参与后续机制
