...
- 火、水元素追击在累计后延迟 `5tick` 统一判定；同类多次累计会选中对应元素块最近的不同玩家，不会对同一最近玩家重复释放
- 火元素追击生成内径 `5m`、外径 `10m` 环形 AOE；水元素追击生成半径 `5m` 圆形 AOE；判定前 `0.5s` 显示对应形状预兆
- 元素追击命中时附加易伤，并按对应元素块为击退源执行击退
- 混沌之风面对击退源时击退距离翻倍，背对击退源时减半；混沌之逆风相反
- 持有风类状态的玩家吃到元素追击击退时，风类状态被移除，并累计风分摊追击
- 风分摊追击在累计后延迟 `5tick` 统一判定；每次选中距离水元素块最近的不同玩家，释放半径 `6m` 的 `2` 人分摊；判定前 `0.5s` 显示圆形预兆
- 风分摊少于 `2` 人时记录失败并击杀目标
- 机制伤害命中已有易伤玩家时即死，否则造成伤害并赋予 `3s` 易伤
