...
- 第二组与第三组黑洞三条连线持续三轮射线，最后一轮后黑洞和连线消失；`121000ms` 第四组黑洞先让两个相邻黑洞连线并在 `7100ms` 后射线消失，同时触发场外凯夫卡再次出现，再让剩余一个黑洞连线并在 `7100ms` 后射线消失
...
- 本色出演的我从场外凯夫卡向场中心释放宽 `10m`、长 `50m` 的矩形即死范围；后半段追加本色出演的我在 `128100ms` 凯夫卡出现，`130100ms` 开始读条 `5000ms`，`135100ms` 判定
- `70600ms` 后半段诅咒敕令和本色出演的我开始，`76600ms` 第二次暴雷，`88100ms` 重复第二组黑洞，`107100ms` 场外再次生成凯夫卡并在 `113100ms` 开始响亮亮耳光读条，`111100ms` 卡奥斯随机读条经度聚爆或纬度聚爆
- 经度聚爆/纬度聚爆读条 `5s`，锁定卡奥斯当前位置和该位置指向当时 `ST` 的方向，卡奥斯转向该方向，读条结束与 `2s` 后分别结算两片相对 `90°`、半径 `60m` 的即死扇形；经度先前后再左右，纬度先左右再前后
- 所有 AOE 在结算前 `0.5s` 显示预兆
- 当前版本不实现 Bot 跑法，Bot 在该战斗中不会由 content 主动移动
