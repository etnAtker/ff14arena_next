...
- 半径 `20m` 的圆形场地，首领目标圈隐藏
- 场中无固定 Boss，`卡奥斯` 与 `艾克斯迪斯` 分别以表现用敌人标记追踪 `ST` 与 `MT`
...
- 第二组黑洞三条连线持续三轮射线，最后一轮后黑洞和连线消失
- 暴雷由艾克斯迪斯读条并在读条期间停止移动，结束后与 `3s` 后分别选择最近玩家结算 `5m` 圆形范围；非 T 命中即死
- 诅咒敕令由卡奥斯读条并在读条期间停止移动，读条开始时随机锁定一名机制有效玩家方向，结束后结算 `180°`、半径 `30m` 即死扇形
- 本色出演的我从场外凯夫卡向场中心释放宽 `10m`、长 `40m` 的矩形即死范围
